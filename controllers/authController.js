...
async function getUserDetail(contact) {
    try {
        // let phone = `(${contact.isdCode}) ${contact.mobileNumber}` 
        let phone = contact.mobileNumber
        let existingPractitioner = await getUserData(phone);
        console.log(existingPractitioner);
        if (existingPractitioner.length < 1) {
            ...
            console.log(JSON.parse(existingPractitioner[1].res_text_vc)?.location);
            let userDetail = {
                profile: {
                    "user_name": user_name,
                   // "user_email": email[0].value,
                    ...
                    "locationId" : JSON.parse(existingPractitioner[1].res_text_vc).location || null
                }, dataValues: {}
            };
...
async function getUserData(mobileNumber) {
    ...
    // let response = await axios(config.baseUrl+`PractitionerRole?practitioner=Practitioner/${practitionerResource[0].res_id}`);
    // console.log(response.data.entry[0].resource)
    // console.log(roleResource[0])
