...
router.post("/",
    [
        check('firstName').notEmpty().isAlpha().isLength({ min: 3, max: 50 }),
        check('lastName').optional({nullable: true, checkFalsy: true}).isAlpha().isLength({ min: 3, max: 50 }),
        check('active').notEmpty().isBoolean(), 
        check("mobileNumber").notEmpty().isLength({ min: 11, max: 17 }),
