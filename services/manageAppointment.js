...
let setApptData = async function (resType, reqInput, FHIRData, reqMethod, reqQuery) {
    try {
        ...
        if (["post", "POST", "PUT", "put"].includes(reqMethod)) {
            for (let apptData of reqInput) {
                ...
                if (locationResource.data.total && locationResource.data.total == 1) {
                    ...
                    let noneExistDataSlot = [
                        { "key": "start", "value": apptData.slot.start },
                        { "key": "actor", "value": 'Schedule/' + apptData.scheduleId }
                    ];
